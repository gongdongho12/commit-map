---
title: "가오슝 3박 4일 · 일일 투어와 소류구 스노클링"
date: 2026-09-24
endDate: 2026-09-27
country: "대만"
tripType: ["food", "sightseeing", "nature", "adventure", "shopping"]
draft: false
flights:
  - flightNo: "7C6123"
    date: "2026-09-24"
    direction: "outbound"
    airline: "제주항공"
    from: "GMP"
    to: "KHH"
    note: "사용자 확정 항공편. 출도착 시간은 예약표로 최종 확인"
  - flightNo: "BR172"
    date: "2026-09-27"
    direction: "return"
    airline: "에바항공"
    from: "KHH"
    to: "ICN"
    note: "사용자 확정 항공편. 출발은 김포, 귀국은 인천"
expenses:
  - category: flight
    title: "제주항공 김포 → 가오슝"
    date: "2026-09-24"
    amount: 184230
    currency: "KRW"
    relatedFlightNo: "7C6123"
    note: "사용자 제공 항공권 금액"
  - category: flight
    title: "에바항공 가오슝 → 인천"
    date: "2026-09-27"
    amount: 348300
    currency: "KRW"
    relatedFlightNo: "BR172"
    note: "사용자 제공 항공권 금액"
  - category: hotel
    title: "URBAN HOTEL33 3박"
    date: "2026-09-24"
    amount: 197658
    currency: "KRW"
    quantity: 3
    unit: "박"
    linkedLocation: "URBAN HOTEL33"
    note: "9/24 체크인~9/27 체크아웃, 3박 전체 금액이며 박당 금액이 아님"
  - category: activity
    title: "클룩 가오슝 일일 투어 (한국어 가이드)"
    date: "2026-09-25"
    amount: 54600
    currency: "KRW"
    bookingSource: "Klook"
    note: "9/25 전일 투어 확정(컨딩 제외). 미려도역 집결, 불광산·연지담·보얼·영국영사관 등 전용버스 코스"
locations:
  - name: "김포국제공항"
    lat: 37.5583
    lng: 126.7906
    type: airport
    order: 1
    visitDate: "9/24"
    country: "한국"
    note: "7C6123 출국"
  - name: "가오슝국제공항"
    lat: 22.5771
    lng: 120.3500
    type: airport
    order: 2
    visitDate: "9/24, 9/27"
    note: "MRT R4, 미려도에서 오렌지선 환승"
  - name: "URBAN HOTEL33"
    lat: 22.6300
    lng: 120.3113
    type: hotel
    order: 3
    visitDate: "9/24-9/27"
    link: "https://www.google.com/maps/search/?api=1&query=URBAN+HOTEL33+Kaohsiung"
    note: "확정 숙소 3박. 신이국소역 인근 대략 위치, 입구는 지도 링크로 확인"
  - name: "딘타이펑 한신 아레나점"
    lat: 22.6698
    lng: 120.3023
    type: restaurant
    order: 4
    visitDate: "9/24"
    link: "https://www.hanshin.com.tw/漢神巨蛋/tw/Floor/B1F/鼎泰豐"
    note: "9/24 16:45 대기 접수·이른 저녁. 한신 아레나 B1 쇼핑몰"
  - name: "루이펑 야시장"
    lat: 22.6667
    lng: 120.2990
    type: market
    order: 5
    visitDate: "9/24"
    note: "9/24 19:00 천사지파이·야시장. 딘타이펑 후 도보 이동"
  - name: "미려도역 (포모사 블러바드)"
    lat: 22.6314
    lng: 120.3019
    type: transport
    order: 6
    visitDate: "9/25"
    link: "https://www.krtc.com.tw/KLRT/guide_map"
    note: "9/25 클룩 일일 투어 집결 장소. 지하 돔 오브 라이트(빛의 돔)"
  - name: "연지담 · 용호탑"
    lat: 22.6806
    lng: 120.2917
    type: attraction
    order: 7
    visitDate: "9/25"
    note: "9/25 오전 투어 코스. 가오슝 대표 호수 풍경과 용호탑"
  - name: "불광산 불타기념관"
    lat: 22.7565
    lng: 120.4497
    type: temple
    order: 8
    visitDate: "9/25"
    link: "https://www.fgsbmc.org.tw/"
    note: "9/25 투어 핵심 코스. 전 세계 최대 청동 좌불(불광대불) 관람"
  - name: "보얼예술특구"
    lat: 22.6200
    lng: 120.2816
    type: attraction
    order: 9
    visitDate: "9/25"
    link: "https://www.en.pier2.org/transport/"
    note: "9/25 투어 방문. 써니힐(SunnyHills) 펑리수 픽업 및 디자인 샵"
  - name: "다거우 영국영사관"
    lat: 22.6186
    lng: 120.2673
    type: attraction
    order: 10
    visitDate: "9/25"
    note: "9/25 오후 투어 코스. 시즈완 언덕에서 바라보는 가오슝 항구 전망"
  - name: "사랑의 강 (아이허)"
    lat: 22.6234
    lng: 120.2885
    type: attraction
    order: 11
    visitDate: "9/25"
    note: "9/25 투어 코스. 가오슝을 가로지르는 낭만적인 강변 풍경"
  - name: "리우허 야시장"
    lat: 22.6334
    lng: 120.3005
    type: market
    order: 12
    visitDate: "9/25"
    note: "9/25 투어 최종 하차 지점. 해산 후 저녁 식사 및 간식"
  - name: "둥강 선착장"
    lat: 22.4682
    lng: 120.4485
    type: transport
    order: 13
    visitDate: "9/26"
    note: "9/26 소류구행 배편 탑승 및 가오슝 복귀 선착장"
  - name: "소류구 바이샤항 (전기스쿠터 대여)"
    lat: 22.3538
    lng: 120.3807
    type: transport
    order: 14
    visitDate: "9/26"
    note: "소류구 관문 선착장. 항구 바로 앞에서 무면허 전기스쿠터(전동차) 대여"
  - name: "소류구 수기부잠 (秀記浮潛)"
    lat: 22.3549507
    lng: 120.3814185
    type: spa
    order: 15
    visitDate: "9/26"
    link: "https://maps.app.goo.gl/6qvMkMZoN6G9okRk9"
    note: "개인 오리발·스노클 지참 후 짐 보관 & 온수 샤워 이용. 바이샤항 도보 3분"
  - name: "화제석 (꽃병바위, 花瓶岩)"
    lat: 22.3562
    lng: 120.3804
    type: nature
    order: 16
    visitDate: "9/26"
    note: "소류구 대표 바다거북 스노클링 입수 포인트. 수기부잠 바로 인접"
  - name: "소류구 민성로 상점가 (거북이 키링)"
    lat: 22.3529
    lng: 120.3802
    type: shopping
    order: 17
    visitDate: "9/26"
    note: "바이샤항 앞 기념품 거리. 핸드메이드 바다거북 키링·소품 쇼핑"
  - name: "POYA 가오슝 신이점 (寶雅)"
    lat: 22.6305
    lng: 120.3090
    type: shopping
    order: 18
    visitDate: "9/26"
    note: "숙소 인근 드럭스토어. DARLIE(달리) 치약 종류별 구매 및 뷰티 쇼핑"
  - name: "인천국제공항"
    lat: 37.4602
    lng: 126.4407
    type: airport
    order: 19
    visitDate: "9/27"
    country: "한국"
    note: "BR172 귀국"
tags: ["대만", "가오슝", "소류구", "소류도", "전기스쿠터", "바다거북키링", "수기부잠", "스노클링", "바다거북", "클룩일일투어", "한국어가이드", "불광산", "불타기념관", "연지담", "URBANHOTEL33", "천사지파이", "딘타이펑", "리우허야시장", "달리치약", "POYA"]
excerpt: "9/25 클룩 일일 투어로 불광산·연지담·영국영사관을 편안히 둘러보고, 9/26 소류구에서 전기오토바이 드라이브, 개인 장비 바다거북 스노클링(수기부잠 샤워), 거북이 키링 득템 후 시내 쇼핑을 즐기는 가오슝 3박 4일 일정."
---

**첫날 북쪽 맛집(딘타이펑·루이펑), 둘째 날 클룩 일일 투어(불광산·연지담·영국영사관), 셋째 날 소류구 전기오토바이·바다거북 스노클링·거북이 키링 쇼핑**으로 완성했다. 컨딩 투어는 장거리 이동 부담으로 제외(포기)하고, 25일에 **클룩 한국어 가이드 투어**로 가오슝 시내 및 불광산 핵심 명소를 전용버스로 편안하게 관람한다. 26일에는 항구에서 무면허로 탈 수 있는 **전기오토바이(전동차)**를 빌려 섬을 누비고, **수기부잠(秀記浮潛)**에 짐을 맡긴 뒤 개인 오리발과 스노클로 바다거북을 만난다. 샤워 후에는 바이샤항 앞 상점가에서 **소류구 시그니처 바다거북 키링**을 기념품으로 사고, 가오슝으로 돌아와 DARLIE 치약 쇼핑으로 하루를 깔끔하게 완성한다.

시각은 현지 기준이며, 항공 배치는 **7C6123 김포 09:45 → 가오슝 12:00 / BR172 가오슝 15:50 → 인천 19:45** 조회 시간표를 기준으로 했다. 실제 예약표와 다른 경우 공항 이동부터 조정한다. [출국 시간표](https://zbordirect.com/en/low-cost/from-korea/to-taiwan/seoul-kaohsiung), [귀국 시간표](https://zbordirect.com/en/tools/schedule?arrival_city=SEL&departure_city=KHH).

## 일정 요약

| 날짜 | 동선 | 하루의 마무리 |
|---|---|---|
| 9/24 목 | 공항 → URBAN HOTEL33 → 한신 아레나 딘타이펑 → 루이펑 천사지파이 | 20:30 호텔, 다음 날 투어 준비 |
| 9/25 금 | **[확정] 클룩 일일 투어**: 미려도역 집결 → 연지담 → 불광산 불타기념관 → 보얼(써니힐) → 영국영사관 → 사랑의 강 → 리우허 야시장 | 19:30 야시장 저녁 후 호텔 복귀 |
| 9/26 토 | **소류구 3대 퀘스트**: 전기오토바이 렌트 → 수기부잠 샤워 & 바다거북 스노클 → 거북이 키링 겟 → 가오슝 복귀 후 POYA 치약 쇼핑 | 20:30 호텔, 치약 위탁 짐에 포장 |
| 9/27 일 | 아침 → 11시 전 체크아웃 → 점심 → 공항 | 13시 공항 도착, BR172 귀국 |

## 9/25 금 · 클룩 가오슝 일일 투어 (한국어 가이드) 확정

**9/25(금)에 [클룩 가오슝 일일 투어 (한국어 가이드)](https://www.klook.com/ko/activity/22010-day-tour-kaohsiung/)를 확정했다.** 전용 차량과 한국어 가이드가 동행하여 가오슝의 대표적인 역사·문화 명소를 하루에 쾌적하게 연결한다.

* **예약 상품**: [가오슝 일일 투어 (한국어 가이드) - Klook](https://www.klook.com/ko/activity/22010-day-tour-kaohsiung/)
* **집결 장소**: **MRT 미려도역 (포모사 블러바드역)** 지하 '빛의 돔(Dome of Light)'
* **출발 시간**: 통상 08:30~09:00 사이 (바우처의 지정 시각 기준)
* **주요 방문지**: 연지담(용호탑) → 불광산 불타기념관 → 보얼 예술특구 → 다거우 영국영사관 / 치진 → 사랑의 강(아이허) → 리우허 야시장(하차 해산)
* **쇼핑 포인트**: 보얼 예술특구 자유시간에 **써니힐(SunnyHills) 보얼점**에 들러 선물용 프리미엄 펑리수를 바로 픽업할 수 있다.

> [!NOTE]
> **컨딩 제외 및 소류구 26일 이동 효과**
> - **컨딩 투어 제외**: 왕복 4시간 이상 걸리는 컨딩은 이동 피로도를 감안하여 완전히 제외했다.
> - **완벽한 역할 분담**: 25일은 가오슝 핵심 명소(불광산/연지담/보얼/영국영사관)를 버스로 편하게 둘러보고, 26일은 소류구 바다와 섬 라이딩에 집중하여 여행의 완성도를 극대화했다.
> - **출발지 접근성**: 숙소인 URBAN HOTEL33에서 MRT 1정거장(신이국소역→미려도역) 거리로 아침 집결이 매우 수월하다.

## 일정 타임라인

```mermaid
timeline
  title 가오슝 3박 4일 확정 일정
  9/24 도착 : 15시 호텔 체크인
             : 16시 45분 딘타이펑 대기 접수
             : 19시 루이펑 천사지파이
  9/25 투어 : 08시 45분 미려도역 집결
             : 줘잉 연지담 & 불광산 불타기념관
             : 보얼예술특구(써니힐) & 영국영사관
             : 사랑의 강 & 18시 리우허 야시장 하차
  9/26 바다 : 07시 15분 호텔 출발 / 둥강 선착장 이동
             : 소류구 도착 & 전기오토바이 대여
             : 수기부잠 거점 바다거북 스노클 & 온수 샤워
             : 점심 & 바다거북 키링 쇼핑 후 복귀
             : 가오슝 복귀 후 저녁 & POYA 치약 쇼핑
  9/27 귀국 : 11시 전 체크아웃
             : 12시 호텔에서 공항 출발
             : BR172 인천 귀국
```

## 9/24 목 · 체크인 후 딘타이펑과 천사지파이

| 현지 시간 | 할 일 | 이동·대기 배치 |
|---|---|---|
| 12:00∼13:15 | 가오슝 도착·입국·수하물 | 정오 도착 기준 75분 |
| 13:15∼14:15 | MRT로 호텔 이동·짐 보관 | R4 공항 → R10/O5 미려도 환승 → O6 신이국소, 도보 포함 60분 |
| 14:15∼15:00 | 호텔 근처 가벼운 점심 | 딘타이펑을 위해 간단히 먹기 |
| 15:00∼16:00 | 체크인·샤워·휴식 | 도착 직후 북쪽까지 바로 이동하지 않기 |
| 16:00∼16:45 | 호텔 → 한신 아레나 | O6 → 미려도 환승 → R14 쥐단, 역에서 도보 포함 |
| 16:45∼18:30 | 딘타이펑 대기 접수·이른 저녁 | **漢神巨蛋 B1**. 식사 및 백화점 잡화 쇼핑 |
| 18:30∼19:00 | 루이펑 야시장으로 도보 | 중간 상점 구경 포함 30분 |
| 19:00∼19:45 | 천사지파이·야시장 | 天使雞排 루이펑점, 야시장 소품 구경 |
| 19:45∼20:30 | MRT로 호텔 복귀 | 다음 날 일일 투어 바우처 확인 및 휴식 |

[호텔 → 딘타이펑](https://www.google.com/maps/dir/?api=1&origin=URBAN+HOTEL33+Kaohsiung&destination=Din+Tai+Fung+Hanshin+Arena&travelmode=transit) · [딘타이펑 → 루이펑](https://www.google.com/maps/dir/?api=1&origin=Din+Tai+Fung+Hanshin+Arena&destination=Ruifeng+Night+Market&travelmode=walking).

호텔은 **No.33, Minzu 2nd Rd.**, 조회된 체크인은 **15:00**, 체크아웃은 **11:00**다. [호텔 등록 정보](https://www.taiwanstay.net.tw/TSA/web_page/TSA020200.jsp?hohi_id=17023&lang2=en), [체크인·체크아웃 안내](https://www.booking.com/hotel/tw/urban-hotel33.en-gb.html), [MRT 공식 노선도](https://www.krtc.com.tw/KLRT/guide_map).

## 9/25 금 · 클룩 가오슝 일일 투어 (한국어 가이드)

| 현지 시간 | 할 일 | 이동·대기 배치 |
|---|---|---|
| 07:30∼08:15 | 아침 식사 및 외출 준비 | 호텔 주변 간단한 식사 |
| 08:15∼08:45 | 숙소 → 미려도역 이동 | MRT 신이국소역(O6) → 미려도역(O5/R10) 1정거장, 도보 포함 약 20분 |
| 08:45∼09:00 | **미려도역 집결 및 가이드 미팅** | 지하 '빛의 돔' 앞 집결, 전용 차량 탑승 |
| 09:30∼10:45 | **연지담 (줘잉 풍경구)** | 용의 입으로 들어가 호랑이 입으로 나오는 용호탑 관람 |
| 11:30∼13:45 | **불광산 불타기념관** | 세계 최대 청동 좌불(불광대불) 및 전시 관람, 사찰 내 자유 점심 |
| 14:30∼15:30 | **보얼 예술특구 & 써니힐 쇼핑** | 창고 거리 산책 및 **써니힐 보얼점** 펑리수 구매 |
| 15:45∼16:45 | **다거우 영국영사관** | 언덕 위 붉은 벽돌 영사관 건물과 시즈완 항구 파노라마 |
| 17:00∼17:40 | **사랑의 강 (아이허)** | 가오슝의 대표적인 낭만 수변 공간 산책 |
| 18:00∼19:30 | **리우허 야시장 하차 및 저녁** | 투어 종료 후 야시장 구경, 해산물·파파야우유 등 현지 미식 |
| 19:30∼20:00 | 호텔 복귀 및 휴식 | 미려도역에서 신이국소역으로 1정거장 복귀 |

## 9/26 토 · 소류구 3대 퀘스트 & 가오슝 저녁 쇼핑

개인 오리발과 스노클 마스크를 챙겨 소류구로 이동한 뒤, **전기오토바이 드라이브 + 바다거북 스노클링(수기부잠 샤워) + 거북이 키링 쇼핑**을 즐기는 완벽한 하루 코스다.

* **전기오토바이 대여**: 바이샤항(선착장) 바로 앞에서 국제면허 없이 탈 수 있는 **전기스쿠터(電動自行車)**를 대여하여 기동성을 확보한다.
* **스노클 & 샤워 거점**: [小琉球秀記浮潛 (Xiuji Snorkeling Show)](https://maps.app.goo.gl/6qvMkMZoN6G9okRk9)
  * 바이샤항 도보 3분 거리. 개인 장비 지참 시 온수 샤워실, 헤어드라이어, 락커 짐 보관 이용
  * 샵 바로 앞이 소류구 대표 바다거북 명당인 **화제석(꽃병바위, 花瓶岩)** 해변
* **거북이 키링 쇼핑**: 바이샤항 및 민성로(民生路) 상점가에 핸드메이드 뜨개질/가죽/목조 **바다거북 키링(海龜鑰匙圈)** 기념품 샵들이 모여 있다.

| 현지 시간 | 할 일 | 이동·대기 배치 |
|---|---|---|
| 06:30∼07:15 | 가벼운 조식 및 출발 준비 | 수영복 이너 착용, 개인 오리발·스노클·방수팩 지참 |
| 07:15∼08:45 | 숙소 → 둥강 선착장 이동 | 픽업차량 또는 왕복 택시 예약 이용 (약 60~80분) |
| 08:45∼09:30 | 승선권 발권 및 승선 대기 | 실물 티켓 수령 (신분증/여권 지참) |
| 09:30∼10:00 | 둥강 선착장 → 소류구 바이샤항 | 고속 페리 탑승 (약 20~25분 소요) |
| 10:00∼10:30 | **전기오토바이 렌트 & 수기부잠 이동** | 선착장 앞 전동스쿠터 대여 → 수기부잠에 짐 보관 |
| 10:30∼12:30 | **바다거북 스노클링 세션** | 화제석(꽃병바위) 포인트에서 개인 오리발·마스크로 스노클링 |
| 12:30∼13:30 | **수기부잠 온수 샤워 & 환복** | 샵 내 온수 샤워실 이용, 보송한 옷으로 갈아입기 |
| 13:30∼14:30 | **점심 식사 & 바다거북 키링 득템** | 항구 근처 맛집(해산물·망고빙수) 식사 후 상점가에서 거북이 키링 쇼핑 |
| 14:30∼15:00 | 전기오토바이 반납 & 선착장 대기 | 바이샤항 대여소에 스쿠터 반납 후 페리 대기 |
| 15:00∼15:30 | 소류구 → 둥강 선착장 페리 | 오후 배편 탑승 |
| 15:30∼17:00 | 둥강 선착장 → 가오슝 숙소 복귀 | 차량 이동, 호텔 복귀 후 장비 정리 |
| 17:00∼18:30 | 호텔 정돈 및 가벼운 휴식 | 짐 정리 및 편한 쇼핑 복장으로 환복 |
| 18:30∼20:30 | **시내 저녁 식사 & 메인 쇼핑** | 숙소 인근 맛집 저녁 식사 후 **POYA(포야)**에서 달리치약 등 쇼핑 |
| 20:30∼21:30 | 호텔 복귀 & 수하물 최종 패킹 | 치약 및 액체류는 모두 위탁수하물 캐리어에 안전 포장 |

> [!TIP]
> **스노클링 안전 & 쇼핑 팁**
> - **면허 걱정 없는 전기스쿠터**: 대만은 외국인의 일반 오토바이 렌트가 까다롭지만, 소류구의 **소형 전기스쿠터/전동자전거**는 국제면허증 없이 여권 제시만으로 쉽게 대여할 수 있습니다. 헬멧을 착용하고 안전하게 운전하세요.
> - **비행 전 휴식 걱정 없음**: 스쿠버다이빙과 달리 수면 스노클링은 비행 전 감압 휴식(18시간 이상) 제약이 없으므로, 다음 날(9/27 15:50) 인천행 귀국 비행기(BR172) 탑승에 안전 문제가 전혀 없습니다.
> - **치약 수하물 규정**: 대만 인기 쇼핑템인 **DARLIE(달리) 치약**은 젤류로 분류되어 100mL(g) 초과 시 기내 반입이 엄격히 금지됩니다. 구매 후 반드시 호텔에서 위탁수하물(부치는 캐리어)에 넣어 패킹해야 합니다.

## 9/27 일 · 11시 체크아웃, 12시 공항 출발

| 현지 시간 | 할 일 | 마감 기준 |
|---|---|---|
| 08:30∼09:30 | 아침 식사 | 호텔 주변 |
| 09:30∼10:45 | 짐 정리·체크아웃 | **11시 이전 객실 반납** |
| 10:45∼11:45 | 짐 보관 후 근처 이른 점심 | 점심 대기가 길면 공항에서 식사 |
| 11:45∼12:00 | 짐 회수·출발 준비 | 호텔에서 MRT 신이국소역 이동 |
| 12:00∼13:00 | MRT로 가오슝공항 이동 | O6 → 미려도 환승 → R4 공항역, 도보 포함 60분 |
| 13:00∼15:50 | 수속·출국·탑승 | 출발 15:50보다 약 3시간 전 공항 도착 |
| 15:50∼19:45 | BR172 가오슝 → 인천 | 김포 출국, 인천 귀국 |

## 확인된 비용 내역

| 항목 | 금액 | 비고 |
|---|---:|---|
| 7C6123 김포 → 가오슝 | 184,230원 | 제주항공 |
| BR172 가오슝 → 인천 | 348,300원 | 에바항공 |
| URBAN HOTEL33 3박 | 197,658원 | 3박 총액 (박당 약 65,886원) |
| 클룩 가오슝 일일 투어 (1인) | 54,600원 | 9/25 전일 한국어 가이드 투어 |
| **확인 합계** | **784,788원** | 전기스쿠터, 소류구 페리, 수기부잠 시설비, 쇼핑 별도 |
