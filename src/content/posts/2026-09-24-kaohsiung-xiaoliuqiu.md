...
> 3. **가오슝 MRT 개찰구 컨택리스(Touch) 직접 태그 탑승 (유니온페이 완벽 지원!)**:
>    - **가오슝 MRT만의 특급 장점**: 타이베이 지하철과 달리, **가오슝 지하철(KRTC)은 유니온페이(UnionPay / 銀聯) 컨택리스(QuickPass) 직접 태그 승하차를 공식 지원**합니다!
>    - **사용자 카드 100% 활용**: 소지하신 **하나 트래블로그 유니온페이 컨택리스 카드**나 트래블월렛(비자) 실물 카드를 개찰구 상단 전용 리더기에 **교통카드처럼 직접 띡! 찍고 바로 탑승**하실 수 있습니다 (개찰구 통과 시 별도 이지카드 충전 잔액 걱정 제로!).
>    - **실물 이지카드(EasyCard)의 역할**: 가오슝 지하철(MRT)은 유니온페이 트래블로그 카드로 바로 타시고, 구매하신 실물 이지카드(87 TWD)는 **Day 3 동강 ↔ 가오슝 9127D 직행버스(시외버스는 이지카드 필수)** 및 **숙소 앞 PX Mart/편의점 소액 결제**용으로 완벽하게 분담하여 사용하시면 가장 알뜰하고 편리합니다.
>
...
> | **② [메인] 1층 입국장 도착홀<br>(세관 통과 직후 로비)** | **메가은행 (좌측 끝) & 대만은행 (우측 끝) 창구 및 ATM**<br>• 자동문 열리고 나오면 로비 양쪽 끝에 각각 위치 | • **대기줄 회피 꿀팁 ⚡**: 지하철(MRT) 타러 나가는 통로가 좌측 방향이라 대부분의 입국객이 **좌측 메가은행**으로 몰려 줄이 깁니다. 좌측에 사람이 많다면 망설이지 말고 **반대편 우측 대만은행 창구/ATM**으로 가세요! 줄이 거의 없어 초고속 처리 가능합니다.<br>• **원스톱 3분 플로우**: 세관 통과 → 한산한 ATM(우측 대만은행 또는 좌측 메가은행)에서 100 NTD 분할 인출 → 우측 5번 출구 부근 '유나이트 트래블러'에서 이지카드(87 TWD) 수령 → MRT 이동 |
...
>
> #### 5. [비상금 현금 환전 팁] 원화 직환전 금지! 달러(USD) 이중환전 압도적 우위
