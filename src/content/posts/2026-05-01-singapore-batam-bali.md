---
title: "싱가포르 in 발리 out 동남아 이동 여행"
date: 2026-05-01
endDate: 2026-05-10
country: "싱가포르"
countries: ["싱가포르", "인도네시아"]
tripType: ["food", "healing", "business", "shopping", "nature", "adventure"]
locations:
  - name: "싱가포르 창이공항"
    lat: 1.3644
    lng: 103.9915
    visitDate: "5/1"
    type: airport
    order: 1
    country: "싱가포르"
    note: "싱가포르 in"

  - name: "K2 Guesthouse"
    lat: 1.2995
    lng: 103.8532
    link: "https://maps.app.goo.gl/CanW6AkuTbHpAnUt8"
    visitDate: "5/1"
    type: hotel
    order: 2
    country: "싱가포르"
    note: "1박 숙소"
    contents:
      - heading: "숙소"
        text: "싱가포르 첫날 1박 베이스. 도착일에는 짐을 두고 가볍게 시내 산책으로 이어가기 좋은 숙소."
      - heading: "첫날"
        text: "도착일은 이동 피로를 줄이는 날. 체크인 후 부기스, 아랍 스트리트, 하지 레인 쪽을 가볍게 걷고 싱가포르 첫 끼를 먹는 흐름으로 기록."

  - name: "하지 레인"
    lat: 1.3008
    lng: 103.8593
    visitDate: "5/1"
    type: attraction
    order: 3
    country: "싱가포르"
    note: "싱가포르 도착 후 가볍게 걷기 좋은 거리"
    contents:
      - heading: "저녁 산책"
        text: "컬러풀한 숍하우스, 작은 바와 카페가 몰려 있어 긴 비행 뒤 무리하지 않고 분위기만 보기 좋은 코스."

  - name: "하버프론트 페리터미널"
    lat: 1.2640
    lng: 103.8200
    visitDate: "5/2"
    type: transport
    order: 4
    country: "싱가포르"
    note: "싱가포르에서 바탐으로 페리 이동"

  - name: "하버 베이 페리터미널"
    lat: 1.1537
    lng: 104.0100
    visitDate: "5/2"
    type: transport
    order: 5
    country: "인도네시아"
    note: "바탐 도착"

  - name: "Biz Hotel Batam"
    lat: 1.1423
    lng: 104.0064
    link: "https://maps.app.goo.gl/SZc2pBKoFhgjk1gN6"
    visitDate: "5/2"
    type: hotel
    order: 6
    country: "인도네시아"
    note: "1박 숙소, Nagoya New Town T-15"
    contents:
      - heading: "위치"
        text: "Nagoya 쪽에 있는 바탐 1박 베이스. 하버 베이 페리터미널과 Nagoya Hill 주변으로 움직이기 좋은 위치."

  - name: "Nagoya Hill Shopping Mall"
    lat: 1.1457
    lng: 104.0109
    visitDate: "5/2"
    type: shopping
    order: 7
    country: "인도네시아"
    note: "바탐 시내 쇼핑몰과 저녁 식사"
    contents:
      - heading: "바탐 저녁"
        text: "싱가포르보다 느슨한 물가와 분위기를 체감하는 구간. 쇼핑몰에서 환전, 간단한 쇼핑, 로컬 음식이나 해산물 저녁을 묶기 좋음."

  - name: "Mercure Singapore Bugis"
    lat: 1.2995
    lng: 103.8532
    visitDate: "5/3"
    type: hotel
    order: 8
    country: "싱가포르"
    note: "1박 숙소, 부기스 중심"
    contents:
      - heading: "복귀"
        text: "바탐에서 다시 싱가포르로 돌아와 부기스 중심에 체크인. 짧은 싱가포르 재정비 구간."

  - name: "마리나 베이"
    lat: 1.2834
    lng: 103.8607
    visitDate: "5/3"
    type: viewpoint
    order: 9
    country: "싱가포르"
    note: "싱가포르 야경 산책"
    contents:
      - heading: "야경"
        text: "짧게 머무는 싱가포르에서 가장 확실한 밤 산책 코스. 마리나 베이, 머라이언 파크, 쇼핑몰 동선을 가볍게 묶기 좋음."

  - name: "응우라라이 국제공항 (발리)"
    lat: -8.7467
    lng: 115.1672
    visitDate: "5/4"
    type: airport
    order: 10
    country: "인도네시아"
    note: "발리 이동"

  - name: "ibis Bali Kuta"
    lat: -8.7370
    lng: 115.1780
    visitDate: "5/4"
    type: hotel
    order: 11
    country: "인도네시아"
    note: "1박 숙소"
    contents:
      - heading: "발리 첫날"
        text: "공항 근처에서 무리하지 않고 체크인. 쿠타 쪽으로 넘어가기 전 하루 쉬어가는 숙소."

  - name: "Althia Private Capsule Quarters"
    lat: -8.7045
    lng: 115.1765
    visitDate: "5/5"
    type: hotel
    order: 12
    country: "인도네시아"
    note: "1박 캡슐 숙소"
    contents:
      - heading: "캡슐 숙소"
        text: "가볍게 짐을 두고 움직이는 날. 카페에서 쉬거나 리모트 작업, 마사지, 주변 식당 탐색을 넣기 좋은 구간."

  - name: "TRIBE Bali Kuta Beach"
    lat: -8.7125
    lng: 115.1678
    visitDate: "5/6"
    type: hotel
    order: 13
    country: "인도네시아"
    note: "3박 숙소, 쿠타 비치 앞"
    contents:
      - heading: "메인 베이스"
        text: "쿠타 비치 앞에서 3박. 해변 산책, 수영장, 카페, 쇼핑몰, 마사지까지 느슨하게 반복하기 좋은 여행 후반 베이스."

  - name: "사누르 항"
    lat: -8.6755
    lng: 115.2625
    visitDate: "5/7"
    type: transport
    order: 14
    country: "인도네시아"
    note: "누사페니다 다이빙 출발"
    contents:
      - heading: "출발"
        text: "쿠타에서 사누르 항으로 이동해 누사페니다 다이빙 보트에 탑승. 발리 본섬에서 누사페니다로 넘어가는 날."

  - name: "누사페니다 다이빙"
    lat: -8.7275
    lng: 115.5444
    visitDate: "5/7"
    type: nature
    order: 15
    country: "인도네시아"
    note: "누사페니다 보트 다이빙"
    contents:
      - heading: "다이빙"
        text: "발리 남동쪽 누사페니다 주변에서 진행한 다이빙. 조류가 있는 지역이라 보트 브리핑과 가이드 동선을 잘 따라가는 게 중요한 포인트."
      - heading: "기억"
        text: "쿠타에서 쇼핑몰만 도는 날이 아니라, 바다로 나가 완전히 다른 발리의 장면을 본 날."

  - name: "크리스탈 베이"
    lat: -8.7157
    lng: 115.4573
    visitDate: "5/7"
    type: beach
    order: 16
    country: "인도네시아"
    note: "누사페니다 대표 다이빙 포인트 중 하나"
    contents:
      - heading: "바다"
        text: "맑은 수중 시야와 산호 지형으로 유명한 포인트. 당일 컨디션과 조류에 따라 실제 포인트는 달라질 수 있어 누사페니다 다이빙 대표 지점으로 기록."

  - name: "Beachwalk Shopping Center"
    lat: -8.7167
    lng: 115.1695
    visitDate: "5/7"
    type: shopping
    order: 17
    country: "인도네시아"
    note: "다이빙 후 친구와 영화 관람"
    contents:
      - heading: "휴식"
        text: "누사페니다 다이빙을 마치고 쿠타로 돌아와 쉬기 좋은 쇼핑몰. 식사, 카페, 영화관을 한 번에 해결하기 좋음."
      - heading: "영화"
        text: "친구와 Beachwalk 안에서 영화 '살목지'를 봤다. 여행 중간에 쇼핑몰 영화관에서 잠깐 현지 일상처럼 쉰 날."

  - name: "아융강 래프팅"
    lat: -8.4870
    lng: 115.2560
    visitDate: "5/8"
    type: nature
    order: 18
    country: "인도네시아"
    note: "우붓 근교 아융강 래프팅"
    contents:
      - heading: "액티비티"
        text: "우붓 근교의 아융강에서 래프팅. 계곡 사이로 내려가며 발리 내륙의 숲과 강 풍경을 보는 일정."

  - name: "뜨갈랄랑 계단식 논"
    lat: -8.4319
    lng: 115.2793
    visitDate: "5/8"
    type: nature
    order: 19
    country: "인도네시아"
    note: "우붓 북쪽 계단식 논"
    contents:
      - heading: "풍경"
        text: "발리 우붓 쪽을 대표하는 계단식 논 풍경. 바다와 쇼핑몰 중심의 쿠타와 완전히 다른 내륙 분위기."

  - name: "몽키 포레스트"
    lat: -8.5184
    lng: 115.2588
    visitDate: "5/8"
    type: nature
    order: 20
    country: "인도네시아"
    note: "우붓 원숭이 숲"
    contents:
      - heading: "주의"
        text: "우붓 중심의 숲과 사원 구역. 원숭이가 가까이 다가오니 선글라스나 작은 물건은 조심해서 챙기는 게 좋음."

  - name: "발리 마사지"
    lat: -8.7125
    lng: 115.1678
    visitDate: "5/9"
    type: spa
    order: 21
    country: "인도네시아"
    note: "마지막 밤 공항 가기 전 마사지"
    contents:
      - heading: "회복"
        text: "새벽 비행 전 마지막으로 마사지 받고 몸을 풀기. 늦은 밤 공항 이동 전까지 시간을 보내기 좋은 일정."

  - name: "응우라라이 국제공항 라운지"
    lat: -8.7467
    lng: 115.1672
    visitDate: "5/9"
    type: airport
    order: 22
    country: "인도네시아"
    note: "밤 10시쯤 공항 도착, 라운지 대기"
    contents:
      - heading: "대기"
        text: "숙박 없이 마지막 밤을 공항에서 보내는 일정. 5/9 밤에 공항으로 이동해 라운지에서 쉬다가 5/10 새벽 KE 편으로 출국."

  - name: "응우라라이 국제공항 (귀국)"
    lat: -8.7467
    lng: 115.1672
    visitDate: "5/10"
    type: airport
    order: 23
    country: "인도네시아"
    note: "발리 out"

tags: ["싱가포르", "바탐", "Biz Hotel Batam", "발리", "쿠타", "동남아"]
excerpt: "싱가포르로 들어가 바탐 Biz Hotel Batam에서 1박하고, 다시 싱가포르를 거쳐 발리 쿠타에서 느슨하게 마무리한 동남아 이동 여행"
---

# 싱가포르 in 발리 out 동남아 이동 여행

5월 초, 싱가포르로 들어가 바탐을 거쳐 발리에서 나오는 긴 이동 여행. 도시 이동이 많은 앞부분과, 쿠타 비치 근처에서 속도를 늦추는 뒷부분이 대비되는 일정이다.

## 여행 개요

- 기간: 2026년 5월 1일 ~ 10일
- 경로: 싱가포르 → 바탐 → 싱가포르 → 발리
- 숙소: K2 Guesthouse, Biz Hotel Batam, Mercure Singapore Bugis, ibis Bali Kuta, Althia Private Capsule Quarters, TRIBE Bali Kuta Beach
- 메모: 마지막 날은 숙박 없이 5/9 밤 마사지 후 공항 라운지에서 쉬다가 5/10 새벽 KE 편으로 출국

## 일정 요약

| 날짜 | 지역 | 숙소/이동 |
|------|------|----------|
| 5/1 | 싱가포르 | 창이 도착, K2 Guesthouse 체크인, 하지 레인 산책 |
| 5/2 | 바탐 | 페리 이동, Biz Hotel Batam, Nagoya Hill 저녁 |
| 5/3 | 싱가포르 | 싱가포르 복귀, Mercure Singapore Bugis, 마리나 베이 야경 |
| 5/4 | 발리 쿠타 | 발리 이동, ibis Bali Kuta 체크인 |
| 5/5 | 발리 | Althia Private Capsule Quarters, 카페/마사지/리모트 작업 |
| 5/6 | 발리 쿠타 | TRIBE Bali Kuta Beach 체크인, 해변 산책 |
| 5/7 | 누사페니다/쿠타 | 누사페니다 다이빙, Beachwalk에서 친구와 영화 '살목지' |
| 5/8 | 우붓 | 아융강 래프팅, 뜨갈랄랑 계단식 논, 몽키 포레스트 |
| 5/9 | 발리 | 마지막 쇼핑, 밤 마사지, 22시쯤 공항 도착, 라운지 대기 |
| 5/10 | 발리 | 새벽 KE 편으로 발리 out |

## 5/1 - 싱가포르 도착

창이공항으로 들어와 첫날은 싱가포르 시내에 가볍게 적응하는 날. K2 Guesthouse에 짐을 두고 부기스, 하지 레인, 아랍 스트리트 쪽을 걸으면 도착일치고 부담이 적다. 저녁은 싱가포르답게 호커센터나 근처 로컬 식당으로 마무리.

## 5/2 - 페리 타고 바탐으로

하버프론트에서 페리를 타고 바탐으로 넘어가는 날. 바탐에서는 Biz Hotel Batam에 1박한다. 숙소가 Nagoya 쪽이라 하버 베이 페리터미널에서 이동하기 좋고, Nagoya Hill 주변에서 쇼핑몰, 마사지, 저녁 식사를 묶기 편하다.

## 5/3 - 싱가포르로 돌아와 부기스

짧은 바탐 1박을 마치고 다시 싱가포르로 복귀. Mercure Singapore Bugis에 체크인한 뒤, 낮에는 부기스와 시티홀 쪽을 천천히 보고 밤에는 마리나 베이 야경까지 이어가기 좋은 날이다.

## 5/4 - 발리 이동

싱가포르 구간을 마치고 발리로 이동. 응우라라이 국제공항에 도착한 뒤 첫 숙소는 ibis Bali Kuta. 이 날은 공항 이동과 체크인만으로도 충분해서 쿠타 주변에서 저녁 먹고 쉬는 정도가 좋다.

## 5/5 - 캡슐 숙소와 느슨한 하루

Althia Private Capsule Quarters에서 1박. 이동이 많은 일정 중간이라 카페에서 쉬거나, 짧게 리모트 작업을 하거나, 마사지로 컨디션을 회복하는 날로 잡기 좋다.

## 5/6 - 쿠타 비치 베이스

여행 후반의 메인 베이스는 TRIBE Bali Kuta Beach. 쿠타 비치가 바로 앞이라 아침 산책, 석양, Beachwalk 쇼핑몰, 호텔 수영장, 카페를 느슨하게 반복하기 좋다.

## 5/7 - 누사페니다 다이빙과 Beachwalk 영화

5/7은 쿠타에서 쉬는 날이 아니라 바다로 나간 날. 사누르 항 쪽으로 이동해 보트를 타고 누사페니다 주변에서 다이빙을 했다. 누사페니다는 발리 본섬과는 물빛과 지형이 확 달라서, 이번 여행에서 가장 액티비티다운 하루였다.

다이빙 후에는 쿠타로 돌아와 Beachwalk에서 친구와 영화 '살목지'도 봤다. 바다에서 하루를 보내고 쇼핑몰 영화관으로 들어가는 흐름이 묘하게 현실적인 여행의 쉼표처럼 남았다.

## 5/8 - 우붓 투어

5/8은 우붓으로 올라간 날. 아융강 래프팅으로 시작해서, 계단식 논 풍경을 보고, 마지막으로 몽키 포레스트까지 묶었다. 전날이 바다였다면 이 날은 완전히 내륙의 발리였다.

아융강 래프팅은 숲과 강을 따라 내려가는 코스라 쿠타의 해변 분위기와 다르게 몸을 쓰는 재미가 있었다. 뜨갈랄랑 계단식 논은 사진으로 많이 보던 발리의 초록색 장면이고, 몽키 포레스트는 우붓 특유의 사원과 숲 분위기까지 같이 느낄 수 있는 코스였다.

## 5/9-5/10 - 마사지, 라운지, 새벽 KE 출국

마지막 날은 숙박 없이 넘어가는 일정. 쿠타 주변에서 마지막 쇼핑과 짐 정리를 마친 뒤, 밤에는 마사지로 시간을 보내고 몸을 풀었다.

마사지 후 밤 10시쯤 응우라라이 국제공항에 도착해 라운지에서 대기. 5/10 새벽 KE 편으로 발리를 떠나는 흐름이라, 실제 여행의 마지막 밤은 호텔이 아니라 공항 라운지에서 마무리한 셈이다.
