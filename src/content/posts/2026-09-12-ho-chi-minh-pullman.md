...
locations:
  - name: "떤선녓국제공항"
    ...
    order: 1
    visitDate: "9/12-9/13"
    note: "새벽 도착 후 포하 식사·06시 마사지로 이동. 공개 시간표상 03:35 도착, 실제 예약표 대조"
  - name: "포하 (PHỞ HÀ)"
    lat: 10.7719657
    lng: 106.7052281
    type: restaurant
...
**포하 새벽 식사 → 06시 AI SPA → 풀만 샤워·짐 보관 → 하탐 환전 → 전쟁박물관 → 12시 체크인 목표 → 낮잠 → 수영장 → 라운지 해피아워**로 움직인다. 이번에는 풀만을 충분히 이용하도록 우체국·냐항응온·카페 아파트를 기본 동선에서 빼고, 밤에는 가까운 부이비엔에서 거리 분위기를 구경하고 일반 바에서 여행객들과 대화할 시간을 남겼다.
...
| 9/12 토 새벽 | 공항 → 포하 → AI SPA | 05:40 식당 출발, **06:00∼07:30 마사지 예약** |
...
| 현지 시간 | 할 일 | 이동·예약 기준 |
|---|---|---|
| 03:35∼04:30 | 도착·입국·수하물·차량 호출 | 조회 항공편 기준. 입국이 늦어지면 식사부터 축소 |
...
**새벽 포하·마사지 비용은 하탐 방문 전 필요**하다. 소액 VND를 미리 준비하거나 공항에서 필요한 만큼 마련하고, 스파의 카드 결제 가능 여부는 예약 채널로 확인한다. Grab에 사용할 카드도 출발 전에 등록한다. 하탐이 닫았거나 환전을 취급하지 않으면 호텔에 가까운 영업 중인 정식 환전 창구를 안내받고, **09:30까지 처리가 안 되면 박물관으로 이동**한다. 다른 환전소를 연달아 찾아 12시 입실·낮잠을 밀지 않는다.
...
호텔 공식 사이트에 별도로 명시된 **18:00∼20:00 Rooftop Bar Happy Hour는 유료 바 프로모션**이다. 플래티넘 라운지 혜택과 같은 것으로 계산하지 않는다. 또한 공식 라운지 페이지의 **14:30∼17:30 애프터눈 티는 가격이 표시된 별도 상품**이므로 회원에게 무료로 포함된다고 단정하지 않는다. [루프톱 바 공식 안내](https://www.pullman-saigon-centre.com/restaurants-and-bars/rooftop-bar/). 라운지 음식으로 충분하지 않으면 산책할 때 가볍게 저녁을 먹거나 호텔에서 별도 주문한다.
